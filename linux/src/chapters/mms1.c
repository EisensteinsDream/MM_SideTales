...
void MMS1_startChapter()
{
	...
	p_chapter1SpriteTex = PLEX_loadTextureFromFile(320, 960, 255, 255, 255, 255, 0, MMS1_FILEPATH_TEX_CHAPTER1SPRITEPLATE);

	p_mibsOffice = PLEX_loadTextureFromFile(3840, 1644, 255, 255, 255, 255, 0, MMS1_FILEPATH_TEX_MIBSOFFICE);

	p_levineManor = PLEX_loadTextureFromFile(3840, 1644, 255, 255, 255, 255, 0, MMS1_FILEPATH_TEX_LEVINEMANOR);
	p_levineOffice = PLEX_loadTextureFromFile(3840, 1644, 255, 255, 255, 255, 0, MMS1_FILEPATH_TEX_LEVINEOFFICE);

	p_outsideSchool = PLEX_loadTextureFromFile(3840, 1644, 255, 255, 255, 255, 0, MMS1_FILEPATH_TEX_OUTSIDESCHOOL);
	p_outsideSchoolBlurry = PLEX_loadTextureFromFile(3840, 1644, 255, 255, 255, 255, 0, MMS1_FILEPATH_TEX_OUTSIDESCHOOLBLURRY);
}
...
static void p_startIntroA(const size_t pamaleanaSpriteTex, const size_t fontPlate, const size_t label, const size_t bubble, const size_t arrow)
{
	...
	PLEX_SPRITE mcConnimonSprite = PLEX_createSprite(&mcConnimonPlate);
	...
	PLEX_addAnimation(&mcConnimonSprite, 1, PLEX_getGameTime());

	mcConnimonAnim = PLEX_getAnimation(0, &mcConnimonSprite);

	PLEX_addAnimationFrame(80, mcConnimonAnim);

	MMS_createActorPamaleana(PLEX_WIDTHUNITMAX()/2 - 1, PLEX_HEIGHTUNITMAX() * 0.75 + 1, false);

	p_mcConnimon = PLEX_createActor(mcConnimonSprite, PLEX_convRect(PLEX_convPoint(PLEX_WIDTHUNITMAX()/2 - 1, PLEX_HEIGHTUNITMAX() * 0.75 - 6), PLEX_convDimens(2, 4)), false);
...
static void p_startIntroD(const size_t pamaleanaSpriteTex, const size_t fontPlate, const size_t label, const size_t bubble, const size_t arrow)
{
	...
	PLEX_SPRITE kidsSprite = PLEX_createSprite(&kidsPlate);
	...
	PLEX_addAnimation(&pamaleanaSprite2, 1, PLEX_getGameTime());

	pamInCarAnim = PLEX_getAnimation(0, &pamaleanaSprite2);

	PLEX_addAnimationFrame(75, pamInCarAnim);

	PLEX_addAnimation(&kidsSprite, 1, PLEX_getGameTime());
	PLEX_addAnimation(&kidsSprite, 1, PLEX_getGameTime());

	kidsShadowAnim = PLEX_getAnimation(0, &kidsSprite);

	PLEX_addAnimationFrame(152, kidsShadowAnim);

	kidsAnim = PLEX_getAnimation(1, &kidsSprite);
	...
	p_pamaleana2 = PLEX_createActor(pamaleanaSprite2, PLEX_convRect(PLEX_convPoint(PLEX_WIDTHUNITMAX() * 0.75, PLEX_HEIGHTUNITMAX() * 0.5), PLEX_convDimens(4, 2)), false);

	p_kids = PLEX_createActor(kidsSprite, PLEX_convRect(PLEX_convPoint(PLEX_WIDTHUNITMAX() * 0.75 + 8, PLEX_HEIGHTUNITMAX() * 0.5), PLEX_convDimens(2, 2)), false);
