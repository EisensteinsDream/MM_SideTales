...
// An odd potentially contraversial addition but PLEX being a C library not a C++ library doesn't have access to the kind of fancy strings that clases and objects provide. This problem is corrected by usage of 'fake strings' which are stucts that function as strings with caps. IE. PLEX_FAKESTRING_10 can be no longer than 10 characters. These can be converted to c-strings using the PLEX_fts macro. Also, unlike regular C-strings, these are serializable.

// NOTE: there will be a lot more in this unit to do various string operations.
...
{
#endif
	// functions to create a fake string from a c-string
	...
	// functions to return a fake string's length from a c-string. This is much more efficient than strlen because fake strings contain their length.
	...
	// functions to create a fake string from a number
	...
	// functions that create a blank fakestring (ie. the first character is the null terminator)
	...
	// functions that add a unicode character to a fakestring. the clen is the byte length of the unicode character. This is UTF-8. So the individual symbol lengths are variable. These functions don't modify the string but return a modified version of the string.
	...
	// functions that merge two strings together. It is always in the sequence of string then string2. It does not modify either string but returns the modified version of the combination of both strings.
	...
	// functions that set all strings to lower case
	...
	// functions that compare 2 strings

	extern bool PLEX_compStrings10Int(const PLEX_FAKESTRING_10 a, const PLEX_FAKESTRING_10 b, const bool ignoreCase, const size_t line, const char* filename);
	extern bool PLEX_compStrings25Int(const PLEX_FAKESTRING_25 a, const PLEX_FAKESTRING_25 b, const bool ignoreCase, const size_t line, const char* filename);
	extern bool PLEX_compStrings50Int(const PLEX_FAKESTRING_50 a, const PLEX_FAKESTRING_50 b, const bool ignoreCase, const size_t line, const char* filename);
	extern bool PLEX_compStrings100Int(const PLEX_FAKESTRING_100 a, const PLEX_FAKESTRING_100 b, const bool ignoreCase, const size_t line, const char* filename);
	extern bool PLEX_compStrings500Int(const PLEX_FAKESTRING_500 a, const PLEX_FAKESTRING_500 b, const bool ignoreCase, const size_t line, const char* filename);

	// figures out whether or not string a starts with string b

	extern bool PLEX_startsWithString10Int(const PLEX_FAKESTRING_10 a, const PLEX_FAKESTRING_10 b, const bool ignoreCase, const size_t line, const char* filename);
	extern bool PLEX_startsWithString25Int(const PLEX_FAKESTRING_25 a, const PLEX_FAKESTRING_25 b, const bool ignoreCase, const size_t line, const char* filename);
	extern bool PLEX_startsWithString50Int(const PLEX_FAKESTRING_50 a, const PLEX_FAKESTRING_50 b, const bool ignoreCase, const size_t line, const char* filename);
	extern bool PLEX_startsWithString100Int(const PLEX_FAKESTRING_100 a, const PLEX_FAKESTRING_100 b, const bool ignoreCase, const size_t line, const char* filename);
	extern bool PLEX_startsWithString500Int(const PLEX_FAKESTRING_500 a, const PLEX_FAKESTRING_500 b, const bool ignoreCase, const size_t line, const char* filename);

	// functions to check if number
	...
	// creates an error message for file load errors
