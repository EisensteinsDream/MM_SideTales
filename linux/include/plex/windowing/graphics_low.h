...
// NOTE: There is also a high level graphics system in the graphics unit and header. This is not for drawing functions or other directly usable graphics function you want the graphics.h and graphics.c files NOT graphics_low. This is basically for the branching between different renderers. Also, defaults to "SDL hardware". but will eventually have modules for various opengl versions and directX. There is a broken incomplete module for gl 1.4. You can switch it over by defining the PLEX_TYPE_GL_1_4 but it's pointless because it's not yet functional. There are functions in here but typically you never want to call them. Eventually, also, I'll probably instead of having macros to switch modules but a realtime system.
